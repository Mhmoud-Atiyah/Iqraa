...
std::string iqraaDir =  "/media/mhmoud/Global/MEA/Iqraa/data/";
...
private:
     ...
     */
    static std::string extractPageCount(const std::string &input)
    {
        std::string number;
        for (char c : input)
        {
...
public:
    ...
    int open(const std::string &bookId)
    {
        /* 0. Construct URL */
        std::stringstream seed;
        seed << "https://www.goodreads.com/book/show/" << bookId;
        /* 1. Cache the web page local temporary */
        const std::string filePathHTML = "open.html";//TODO: Change path for multi threads
        Iqraa::retrievePage(seed.str(), filePathHTML);
        ...
        /* 3. Open JSON Handler */
        const std::string filePathJSON = cacheDir + bookId + ".json";
        std::fstream outJson;
        ...
        outJson << "{";
        outJson << "\"id\": "
                << "\"" << bookId << "\"," << std::endl; // Book ID For Data Manipulation
        outJson << "\"coverSrc\": "
        ...
        outJson << "\"pagesCount\": "
                << "\"" << Iqraa::extractPageCount(bookData->select("p[data-testid='pagesFormat']")->to_text()) << "\"," << std::endl;
        outJson << "\"readCount\": "
                << "\"" << bookData->select("div[class='RatingStatistics__meta']")->at(1)->select("span[data-testid='ratingsCount']")->to_text() << "\"," << std::endl;
        outJson << "\"pubDate\": "
    ...
    int search(const std::vector<std::string> &seed)
    {
        ...
        const std::string filePathJSON = cacheDir + "search: " + Iqraa::clearStr(query, "+", " ") + ".json";
